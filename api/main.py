...
def profile_to_dict(profile: UserProfile) -> Dict:
    return {
        ...
        "inferred_skills": getattr(profile, '_inferred_skills', []),
    }
...
async def get_recommendations(
    ...
    return RecommendationResponse(
        ...
        profile_summary={
            ...
            "inferred_skills_count": len(getattr(profile, '_inferred_skills', [])),
        },
    )
...
# NOTE: /api/v1/jobs/new MUST be defined BEFORE /api/v1/jobs/{job_id}
# Otherwise FastAPI treats "new" as a job_id path parameter → 404.
@app.get("/api/v1/jobs/new")
async def get_new_jobs_endpoint(
    since_hours: int = Query(24, ge=1, le=168),
    limit: int = Query(50, ge=1, le=200),
):
    """Jobs scraped in the last N hours — for the New Today screen.

    Prioritizes Internshala (India internships) and India-located jobs.
    Falls back to any recently scraped jobs.
    """
    ...
    # Fetch recently scraped jobs, prioritizing Internshala and India
    cursor.execute("""
        SELECT * FROM jobs
...
# Default profile baked in — no resume upload needed for personal use
from recommendation.engine import UserProfile as _UserProfile

_DEFAULT_PROFILE = _UserProfile(
    name="Manikandan S",
    skills=[
        "python", "pytorch", "tensorflow", "scikit-learn", "fastapi",
        "sqlite", "c++", "machine learning", "deep learning",
        "knowledge distillation", "computer vision", "nlp",
        "competitive programming", "data structures", "algorithms",
    ],
    experience_years=0,
    education=["b.tech", "computer science", "sastra university"],
    job_titles=[
        "software engineer", "ml engineer", "research intern",
        "data scientist", "backend engineer", "ai engineer",
    ],
    locations_preferred=["india", "bangalore", "chennai", "hyderabad",
...
@app.get("/api/v1/recommendations/default")
async def get_default_recommendations(
    limit: int = Query(50, ge=1, le=200),
    min_score: float = Query(0.0, ge=0, le=1),
):
    """Recommendations using the baked-in profile. No user_id or resume needed."""
    engine = RecommendationEngine()
    engine.set_profile(_DEFAULT_PROFILE)
    matches = engine.recommend_jobs(limit=limit, min_score=min_score)
    ...
    return {
        ...
        "profile_summary": {
            "skills_count": len(_DEFAULT_PROFILE.skills),
            "top_skills": _DEFAULT_PROFILE.skills[:5],
            "experience_years": _DEFAULT_PROFILE.experience_years,
            "preferred_types": _DEFAULT_PROFILE.employment_types,
            "inferred_skills_count": len(getattr(_DEFAULT_PROFILE, '_inferred_skills', [])),
        },
    }


# /api/v1/jobs/new is now defined above /api/v1/jobs/{job_id} — see line ~370