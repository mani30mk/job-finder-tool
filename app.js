...
function restore() {
  try {
    ...
    if (d) {
      ...
      if (!d.activeFilter) d.activeFilter = 'all';
      state = d;
      renderCards();
      updateCounts();
    }
...
async function doSearch(fresh) {
  ...
  if (fresh) { state.jobs = []; state.page = 1; state.activeFilter = 'all'; state.activeTab = 'r'; }
...
function renderCards() {
  ...
  if (!wrap) { console.error('[HUNTR] Cards container not found'); return; }
  ...
  console.log('[HUNTR] Rendering', jobs.length, 'jobs, tab:', state.activeTab, 'filter:', state.activeFilter);
  ...
  try {
    ...
    console.error('[HUNTR] Render error:', e);
    wrap.innerHTML = `<div class="empty"><div class="empty-icon">⚠</div>Error rendering jobs. Check console.</div>`;
  }
}
  ...
  <div class="card-top">
    <div class="card-left">
      ...
      <div class="card-meta">
        ...
        ${!hasUrl && j._src === 'Gemini' ? '<span class="badge" style="background:rgba(255,204,0,.07);color:var(--warn);border:1px solid rgba(255,204,0,.2)">⚠ link unverified</span>' : ''}
      </div>
    </div>
...
function clearAll() {
  ...
  state.jobs = []; state.page = 1; state.hasMore = false; state.activeFilter = 'all'; state.activeTab = 'r';
