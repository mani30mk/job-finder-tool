...
class CacheManager:
    ...
    def get_profile(self, file_path: str) -> Optional[Dict]:
        ...
        file_hash = self._hash(Path(file_path).read_text())
        conn = sqlite3.connect(self.cache_path)
        cursor = conn.cursor()
    ...
    def save_profile(self, file_path: str, profile: Any):
        ...
        cursor.execute(
            ...
            (file_hash, json.dumps(profile_dict), datetime.now())
        )
        conn.commit()
        conn.close()
    ...
    def save_embedding(self, text: str, model_name: str, embedding: List[float]):
        ...
        cursor.execute(
            ...
            (text_hash, pickle.dumps(embedding), model_name, datetime.now())
        )
        conn.commit()
        conn.close()
    ...
    def save_job(self, job_id: str, job: Dict, source: str = "web"):
        ...
        cursor.execute(
            ...
            (job_id, json.dumps(job), source, datetime.now())
        )
        conn.commit()
        conn.close()
