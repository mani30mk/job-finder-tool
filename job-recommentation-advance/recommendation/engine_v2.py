...
- Semantic skill/title matching with embeddings
...
class RecommendationEngine:
    ...
    def load_resume(self, filepath: str) -> UserProfile:
        ...
        if cached:
            self.profile = UserProfile(**cached)
    ...
    def _fetch_jobs(self) -> List[Dict]:
        """Fetch jobs from database and web sources."""
        ...
        try:
            if keywords:
                ...
                query = f"SELECT * FROM jobs WHERE is_active = 1 AND ({where_keywords}) ORDER BY posted_date DESC LIMIT 500"
                cursor.execute(query, params)
            else:
                cursor.execute("SELECT * FROM jobs WHERE is_active = 1 ORDER BY posted_date DESC LIMIT 500")
        ...
        if self.web_searcher:
            try:
                ...
                # Cache web jobs
                for job in web_jobs:
                    self.cache.save_job(job["id"], job, job.get("source", "web"))
            except Exception as e:
    ...
    def _score_location(self, job: Dict) -> float:
        ...
        if self.profile.remote_preference == "remote":
            return 1.0 if "remote" in job_location else 0.2
        elif self.profile.remote_preference == "hybrid":
            return 1.0 if "hybrid" in job_location or "remote" in job_location else 0.5
        elif self.profile.remote_preference == "any":
            return 0.8
        if self.profile.locations_preferred:
            for loc in self.profile.locations_preferred:
                if loc.lower() in job_location:
                    return 1.0
            if any(w in job_location for w in ["remote", "worldwide", "anywhere"]):
                return 0.8
            return 0.3
        return 0.7
    ...
    def _generate_reasons(self, match: JobMatch, matched_skills: List[str]) -> List[str]:
        ...
        if match.type_match_score >= 0.8:
            reasons.append(f"Matches preferred type ({job.get('employment_type', '')})")
    ...
    def _generate_explanation(self, match: JobMatch, matched_skills: List[str]) -> str:
        ...
        if match.title_match_score >= 0.8:
            parts.append(f"The {job.get('title', 'role')} title closely matches your previous positions.")
        elif match.title_match_score >= 0.4:
            parts.append(f"The {job.get('title', 'role')} title is somewhat related to your background.")
        
        # Skills
        if matched_skills:
            top = ", ".join(matched_skills[:4])
            parts.append(f"Your profile shows expertise in {top}, which are key for this role.")
        ...
        if match.location_match_score >= 0.8:
            ...
            parts.append(f"The {loc} location matches your preferences.")
