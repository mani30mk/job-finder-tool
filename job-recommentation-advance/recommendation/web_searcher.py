...
class WebJobSearcher:
        ...
        for source in sources:
            try:
                if source == "linkedin":
                    ...
                    jobs = self._search_greenhouse(query)
                elif source == "lever":
                    jobs = self._search_lever(query)
                else:
                    continue
        ...
        try:
            ...
            for card in soup.find_all("div", class_="base-card"):
                try:
                    ...
                    job = {
                        "id": f"li_{hash(title.text if title else '')}",
        ...
        try:
            ...
            for card in soup.find_all("div", class_=re.compile("job_seen_beacon|slider_container")):
                try:
                    ...
                    job = {
                        "id": f"indeed_{hash(title.text if title else '')}",
                        "title": title.text.strip() if title else "Unknown",
                        "company": company.text.strip() if company else "Unknown",
                        "location": location_tag.text.strip() if location_tag else "Remote",
                        "description": "",
    ...
    def _search_greenhouse(self, query: str) -> List[Dict]:
        """Search Greenhouse board jobs."""
        # Greenhouse requires company-specific boards; this is a generic search pattern
        return []

    def _search_lever(self, query: str) -> List[Dict]:
        """Search Lever board jobs."""
        # Lever requires company-specific boards
        return []
